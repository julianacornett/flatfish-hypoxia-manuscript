...
ES_Combined <- merge(ES_DO, All_Temp)
```
...
es_correlation <- ggplot(ES_Combined, aes(x = Temp, y = DO_mgL, color = Station_Code)) + 
  scale_color_manual(values = palette_viridis) +
  geom_point(alpha = 0.25) +
  geom_smooth(method = "lm", se = FALSE) +
  stat_cor(aes(label = paste(..rr.label.., ..p.label.., sep = "*`,`~")), size = 3, label.x = 5) +
  theme_pubr() +
  ylab("Dissolved Oxygen (mg/L)") + 
  xlab("Temperature (\u00B0C)") +
  geom_text(data=ES_Combined[1, ], label="C", vjust=-21, color = "black", size = 5, check_overlap = TRUE) 
es_correlation
```
